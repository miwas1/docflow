...
def build_settings(**overrides) -> ClassifierSettings:
    payload = {
        ...
        "CLASSIFIER_LABEL_DESCRIPTIONS_JSON": {
            ...
            "receipt": "Receipt issued by a cashier after payment.",
            "unknown_other": "Unknown or uncategorized document.",
        },
    }
...
def test_classifier_settings_parse_label_descriptions_and_cache_defaults() -> None:
    ...
    assert settings.classifier_model_name == "answerdotai/ModernBERT-base"
    ...
    assert settings.classifier_label_descriptions["invoice"].startswith("Invoice")
