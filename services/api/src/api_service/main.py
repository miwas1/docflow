...
def create_app() -> FastAPI:
    ...
    @app.post("/v1/documents:upload", status_code=202, response_model=AcceptedUploadResponse)
    def upload_document(
    ...
    @app.get("/internal/webhooks/jobs/{job_id}/dispatch", response_model=WebhookDispatchResponse)
    ...
    def fetch_operator_jobs(
        ...
        session: Session = Depends(get_db_session),
    ) -> dict[str, OperatorDashboardSummaryResponse | list[OperatorJobListItemResponse]]:
        return {
    ...
    @app.get("/internal/operator/jobs/{job_id}", response_model=OperatorJobDetailResponse)
    ...
    def render_operator_dashboard(_: str = Depends(get_authenticated_operator)) -> HTMLResponse:
        template_path = Path(__file__).resolve().parent / "templates" / "operator_dashboard.html"
